...
const SearchScreen = () => {
  ...
  return (
    <BaseScreen title="Vegetables and Fruits" subtitle="All items are currently in store">

        <ScrollView contentContainerStyle={CartStyles.scrollContainer}>
          {cartItems.map((item) => (
            <TouchableOpacity key={item.id}>
              <View style={[CartStyles.cartItem, {backgroundColor: generateRandomColor()}]}>
              <Image source={{ uri: item.image }} style={[CartStyles.productImage, {borderRadius:50}]} />
              <View style={CartStyles.productDetails}>
                ...
                <Text style={CartStyles.productQuantity}>⚖ {item.quantity} Kgs</Text>
              </View>
